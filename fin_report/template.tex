...
\subsection{운동방정식 유도}
약 3~4주차에 걸쳐서 아래의 식을 유도하였다. 자세한 유도과정은 부록 \cref{sec:struct-model}에 수록하였다.
\begin{equation}
...
\begin{figure}[h!]
    ...
    \end{subfigure}
    \caption{$x(t) = Xe^{-\zeta w_{n}t}\cos(\sqrt{1-\zeta^2}w_{n}t-ϕ)$\\X = 0.18813,\(\zeta\)=0.01896,\(w_n\) = 19.9328, \(\phi\)=0.1525}
    \label{fig:week1}
\end{figure}
\subsection{2주차}
...
평균 순항속도\(U\)에 따라서 시스템의 \(\lambda\)\footnote{eigenvalue}가 달라진다. 켤레를 제외한, 두개의 고유한 고유값들 중 하나라도 root-locus plot
에서 RHS\footnote{Right half plane}으로 넘어가면 시스템은 발산하므로,이러한 현상을 예방하기 위한 허용된 최대속도를 root locus plot을 사용하여
탐구해보았다.
\begin{figure}[h!]
    \centering
...
재조사하여 보다 안정적인 설계를 해야함 또한 알 수 있었다.
\subsection{week4}
...
해당 탐구에서는 간단하게 impulse,multisine 함수를 수치해석기법(ode45)를 사용하여 선형과 비선형 거동의 차이를 아래와 같이 보였다.
\begin{figure}[H]
    \centering
...
\end{figure}
보이는 바와 같이, 초기단계에서는 두개가 별 차이가 없으나, 시간이 갈수록 그 차이가 점점 커짐을 알 수 있다. 하지만, 주기적으로 힘이 작용하는 sine함수와 달리
