...
\(V_{\max}\) 이상으로 확보하기 위해 무게중심--탄성중심 간격 \(d\)  
\end{itemize}
4주차에서 보였듯 이 운용 영역에서는 비선형 경화 효과가 충분히 작아, 선형 해석만으로
설계 검증이 타당하다. 위 세 조건을 만족하는 권장값은 \cref{tab:final}에 정리하였다.
\begin{table}[H]
    \centering
    ...
    \begin{tabular}{lll}
        ...
        설계값               & 근거 (주차)                 & 권장 방향 / 값                          \\
        \midrule
        간격 \(d\)           & 플러터 한계 (5주)            & \(d \le 2.5203\SI{}{\centi\meter}\;(V_f=\qty{29.633}{m/s} > V_{\max} )\) \\
        공진주파수\tablefootnote{위에 서술한 Von Karman식에 $V_f$값 대입시 값 감소가 뚜렷해지는 지점을 $w_d_crit$로 지정하였다} \(w_d\)    & 로우패스 대역 (3주)          & $w_d$ $\geq $13.55223\SI{}{\hertz}      \\
        강성 \(k\)          & 질량--강성 trade-off (1·2주) & \(k \gtrsim \SI{5}{\newton\per\metre}\) \\
