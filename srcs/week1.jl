...
# ╔═╡ 44099108-4887-45f5-818c-8c1e258249f0
using CSV, DataFrames, Plots, FFTW, DSP, CurveFit, ForwardDiff, Statistics, LinearAlgebra,LaTeXStrings,PlutoUI

# ╔═╡ f3d44c6a-0090-43a2-93e1-f2cdd16bb6cd
md"""
# WEEK 1
"""

# ╔═╡ 2c6c9ec3-2c74-4dd6-aea3-5481301d9185
df = CSV.read("./resources/week1/week1_wing_gvt_data.csv",DataFrame);

# ╔═╡ b1041ed1-092f-4ecb-832a-95389f058ba1
begin
	t = df[!, "Time (s)"]
	x = df[!, "Displacement (m)"]
	...
	plot(t, x,
	    ...
	    title  = "Week 1 Wing GVT — Free Vibration",
	    legend = false,
	    lw     = 1.5,
	    color  = :steelblue
	)
...
# ╔═╡ d9433d2e-1be4-4846-917d-164cc5fb88b6
begin
	N  = length(t)
	dt = t[2] - t[1]
	fs = 1.0 / dt                  # sampling frequency (Hz)
	# --- Step 1: FFT spectrum (before filter) ---
	X     = abs.(fft(x))[1:N÷2]
	freqs = (0:N÷2-1) .* (fs / N)
	
	dominant_freq = freqs[argmax(X)]
		# --- Step 2: Butterworth band-pass filter ---
margin = 2.0
f_low  = max(0.1, dominant_freq - margin)
f_high = dominant_freq + margin

nyquist    = fs / 2.0
bpf        = digitalfilter(Bandpass(f_low / nyquist, f_high / nyquist), Butterworth(4))
x_filtered = filtfilt(bpf, x)
		# --- Step 3: Spectrogram ---
	win_size = 128
	overlap  = win_size ÷ 2
	sg       = spectrogram(x, win_size, overlap; fs=fs)
		"Dominant frequency: $(round(dominant_freq, digits=3)) Hz (ωd = $(round(dominant_freq*2π,digits=3)) rad/s)"

end

# ╔═╡ b4bd090d-e66a-4b00-8803-aee9e40a0267
begin
	# --- Plot 1: Raw vs Filtered ---
	p1_ = plot(t, x,
	    label  = "Raw",
	    xlabel = "Time (s)", ylabel = "Displacement (m)",
	    title  = "GVT Signal — Raw vs Filtered",
	    lw = 1.5, alpha = 0.8, color = :gray)
	plot!(p1_, t, x_filtered,
	    label = "Butterworth BP ($(round(f_low,digits=1))–$(round(f_high,digits=1)) Hz, order 4)",
	    lw = 2.0, color = :steelblue)

		# --- FFT spectrum after filter ---
	X_filtered = abs.(fft(x_filtered))[1:N÷2]
	
	# --- Plot 2: FFT Spectrum before & after filter ---
	xlim_max = min(fs/2, dominant_freq * 5)
	p2_ = plot(freqs, X,
	    xlabel = "Frequency (Hz)", ylabel = "Amplitude",
	    title  = "FFT Spectrum — Before vs After Filter",
	    legend = true, lw = 1.5, color = :darkorange,
	    xlims  = (0, xlim_max),
	    label  = "Before filter")
	plot!(p2_, freqs, X_filtered,
	    lw = 1.5, color = :steelblue, alpha = 0.8,
	    label = "After filter")
	vline!(p2_, [dominant_freq],
	    linestyle = :dash, color = :red, lw = 1.5,
	    label = "Dominant: $(round(dominant_freq, digits=2)) Hz")
	vline!(p2_, [f_low, f_high],
	    linestyle = :dot, color = :gray, lw = 1.0,
	    label = "Band-pass range")

		# --- Plot 3: Spectrogram ---
	p3_ = heatmap(sg.time, sg.freq, pow2db.(sg.power),
	    xlabel  = "Time (s)", ylabel = "Frequency (Hz)",
	    title   = "Spectrogram",
	    color   = :inferno,
	    ylims   = (0, min(fs/2, dominant_freq * 5)),
	    clims   = (maximum(pow2db.(sg.power)) - 60, maximum(pow2db.(sg.power))))
	hline!(p3_, [dominant_freq],
	    linestyle = :dash, color = :cyan, lw = 1.5,
	    label = "Dominant freq")

	p_ = plot(p1_, p2_, p3_, layout = (3, 1), size = (1000, 1000))

end

# ╔═╡ 98d8c9f2-36bf-4e8b-af3c-d4c7b729c4ea
md"""
 If we neglect bending and torsional coupling,such caused by structural couple,sweep effects etc. then approximate wing as simple as simple beam, moving along with vertical direction. from elastic theory, its well known 
`` \delta = \frac{Wl^3}{3EI}``
where ``I = I_{zz} = \frac{bh^3}{12}`` . a typical airfoil, assume *NACA2412* in this case, has maximum 0.12 ratio of horizontal and vertical length. letting ``h=0.12b`` and ``k = \frac{3EI}{l^3}``.

Therefore, its reducing to first order free vibration analysis. 

``m\ddot{x} + c\dot{x} + kx = \sum_{i=1}^{\infty} H(iw) e^{iwt} = 0`` 

the solution $x(t)$, is given as plot above.
"""

# ╔═╡ 9bcbce35-937b-494b-9a59-76c5d9a63cbb
md"""
by conventional usage of constant in vibration theory, we assume these constant
...
The follwing graph is showing the damping is underdamped, therefore we can express the displacement ``x(t)`` in form of 


``
x(t) = Xe^{-ζw_{n}t}\cos(\sqrt{1-\zeta^2}w_{n}t-ϕ)``

Using curve-fitting algorithms,
"""


# ╔═╡ 7443122e-1607-430c-99d0-b7d5a2984c7f
begin
	t_arr=collect(t)
	x_arr = x_filtered
	f_dom = dominant_freq
...
# p = [X_amp, ζ, ωn, φ]
function fv_model(p, t_vec)
    X_amp, zeta, wn, phi = p[1], p[2], p[3], p[4]
    wd = sqrt(max(1.0 - zeta^2, 1e-10)) * wn
    return @. X_amp * exp(-zeta * wn * t_vec) * cos(wd * t_vec - phi)
end

p0   = [maximum(abs.(x_arr)), 0.05, 2π * f_dom, 0.0]
loss(p) = sum((fv_model(p, t_arr) .- x_arr).^2)
mae(p)  = mean(abs.(fv_model(p, t_arr) .- x_arr))
mse(p)  = mean((fv_model(p, t_arr) .- x_arr).^2)

results    = Dict{String, Vector{Float64}}()
errors     = Dict{String, Float64}()
mse_errors = Dict{String, Float64}()

# ── Algorithm 1: CurveFit.jl (NonlinearSolve auto-polyalgorithm) ──────────────
try
    prob = NonlinearCurveFitProblem(fv_model, p0, t_arr, x_arr)
    sol  = solve(prob)
    results["CurveFit (auto)"]    = sol.u
    errors["CurveFit (auto)"]     = mae(sol.u)
    mse_errors["CurveFit (auto)"] = mse(sol.u)
    println("CurveFit converged=$(isconverged(sol))  MAE=$(round(errors["CurveFit (auto)"],digits=7))")
catch e
    println("CurveFit failed: $e")
end

# ── Algorithm 2: Gauss-Newton (ForwardDiff Jacobian + Tikhonov) ───────────────
function gauss_newton(model, t_vec, y, p_init; maxiter=500, tol=1e-10)
    p = copy(float.(p_init))
    for _ in 1:maxiter
        r  = model(p, t_vec) .- y
        J  = ForwardDiff.jacobian(pp -> model(pp, t_vec), p)
        dp = (J'J + 1e-8 * I) \ (J' * r)
        p .-= dp
        norm(dp) < tol && break
    end
...
try
    p_gn = gauss_newton(fv_model, t_arr, x_arr, p0)
    results["Gauss-Newton"]    = p_gn
    errors["Gauss-Newton"]     = mae(p_gn)
    mse_errors["Gauss-Newton"] = mse(p_gn)
    println("Gauss-Newton        MAE=$(round(errors["Gauss-Newton"],digits=7))")
catch e
    println("Gauss-Newton failed: $e")
end

# ── Algorithm 3: Gradient Descent with Armijo backtracking ────────────────────
function gradient_descent(loss_fn, p_init; maxiter=3000, tol=1e-8)
    p = copy(float.(p_init))
    for _ in 1:maxiter
        g  = ForwardDiff.gradient(loss_fn, p)
        norm(g) < tol && break
        lr = 1e-5
        L0 = loss_fn(p)
        for _ in 1:30
            loss_fn(p .- lr .* g) < L0 - 1e-4 * lr * dot(g, g) && break
            lr *= 0.5
        end
        p .-= lr .* g
    end
...
try
    p_gd = gradient_descent(loss, p0)
    results["Gradient Descent"]    = p_gd
    errors["Gradient Descent"]     = mae(p_gd)
    mse_errors["Gradient Descent"] = mse(p_gd)
    println("Gradient Descent    MAE=$(round(errors["Gradient Descent"],digits=7))")
catch e
...
println("\n=== Fitted Constants ===")
println("Parameter: [X_amp,  ζ,      ωn (rad/s),  φ]")
for (name, p) in results
    println("$name")
    println("  X=$(round(p[1],digits=5))  ζ=$(round(p[2],digits=5))  ωn=$(round(p[3],digits=4)) rad/s fn=$(round(p[3]/(2π),digits=3)) Hz  φ=$(round(p[4],digits=4)) rad ωd=$(round(p[3]*√(1-p[2]^2),digits=4)) rad/s")
    println("  MAE = $(round(errors[name], digits=7)) m")
end

# ── Plot 1: Fitted curves vs filtered signal ───────────────────────────────────
algo_colors = ["CurveFit (auto)" => :steelblue,
               "Gauss-Newton"    => :orange,
...
p1 = plot(t_arr, x_arr,
    ...
    xlabel = "Time (s)", ylabel = "Displacement (m)",
    title  = "Free Vibration Curve Fit  x(t)=Xe^{-ζωₙt}cos(ωdt-φ)")

for (name, col) in algo_colors
    haskey(results, name) || continue
    plot!(p1, t_arr, fv_model(results[name], t_arr),
        label = "$name  MAE=$(round(errors[name],digits=5))",
        lw = 2.0, color = col)
end

# ── Plot 2 & 3: MAE and MSE bar charts side-by-side ──────────────────────────
names_sorted = sort(collect(keys(errors)))
mae_vals     = [errors[n] for n in names_sorted]
mse_vals     = [mse_errors[n] for n in names_sorted]
bar_cols     = [get(Dict(algo_colors), n, :gray) for n in names_sorted]

p2 = bar(names_sorted, mae_vals,
    xlabel = "Algorithm", ylabel = "MAE (m)",
    title  = "Mean Absolute Error",
    color  = bar_cols, legend = false,
    xtickfontsize = 9)

p3 = bar(names_sorted, mse_vals,
    xlabel = "Algorithm", ylabel = "MSE (m²)",
    title  = "Mean Squared Error",
    color  = bar_cols, legend = false,
    xtickfontsize = 9)

p_all = plot(p1, p2, p3, layout = @layout([a; b c]), size = (1000, 900))
end

# ╔═╡ 8ce2732c-e3b8-4888-9f25-7046b0782d98
md"""
Therfore, we estimate 

``X = 0.18813,\zeta=0.01896,w_n = 19.9328, ϕ=0.1525``. 

where ``w_n = \sqrt{\frac{k}{m}}``. 

the value of m and k cant be determined by value.

"""

# ╔═╡ 057655cf-c369-4b36-bdfc-cc77ff1a85f4
begin
	const X_amp = 0.18813
	const zeta0 = 0.01896
	const wn    = 19.9328
	...
	# Initial conditions from the fitted curve at t = 0
	wd0  = sqrt(1.0 - zeta0^2) * wn
	x0   = X_amp * cos(phi)
	xd0  = X_amp * (-zeta0 * wn * cos(phi) + wd0 * sin(phi))
	
	# ── Analytical free-vibration response for arbitrary ζ ────────────────────────
	function free_response(t_vec, zeta, wn, x0, xd0)
	    if abs(zeta - 1.0) < 1e-6          # critically damped
	        A = x0
	        B = xd0 + wn * x0
	        return @. (A + B * t_vec) * exp(-wn * t_vec)
	    elseif zeta < 1.0                   # underdamped
	        wd = sqrt(1.0 - zeta^2) * wn
	        A  = x0
	        B  = (xd0 + zeta * wn * x0) / wd
	        return @. exp(-zeta * wn * t_vec) * (A * cos(wd * t_vec) + B * sin(wd * t_vec))
	    else                                # overdamped
	        sq = sqrt(zeta^2 - 1.0)
	        r1 = (-zeta + sq) * wn
	        r2 = (-zeta - sq) * wn
	        A  = (xd0 - r2 * x0) / (r1 - r2)
	        B  = x0 - A
	        return @. A * exp(r1 * t_vec) + B * exp(r2 * t_vec)
	    end
	end
	
	# ── Sweep ──────────────────────────────────────────────────────────────────────
	zeta_vals = [0.0, 0.1, 0.3, 0.5, 0.7, 0.9, 1.0, 1.5, 2.0]
	t_vec     = range(0.0, 2.0, length = 2000)   # 0–2 s captures decay well
	
	# Color gradient: blue (undamped) → red (overdamped)
	colors = cgrad(:RdYlBu, length(zeta_vals), rev = true)
	
	# ── Plot ───────────────────────────────────────────────────────────────────────
	p = plot(
	    xlabel = "Time (s)",
	    ylabel = "Displacement (m)",
	    title  = "Free Vibration — Effect of Damping Ratio (ωn = $(wn) rad/s)",
	    legend = :topright,
	    size   = (1000, 600),
	    lw     = 1.8
	)
	...
	for (i, zeta) in enumerate(zeta_vals)
	    x_ = free_response(collect(t_vec), zeta, wn, x0, xd0)
	
	    region = zeta < 1.0 ? "underdamped" :
	             abs(zeta - 1.0) < 1e-6 ? "critically damped" : "overdamped"
	
	    label_str = "ζ = $(zeta)  ($region)"
	    plot!(p, t_vec, x_, label = label_str, color = colors[i], lw = 1.8)
	end
	
	# Mark the original fitted ζ
	x_orig = free_response(collect(t_vec), zeta0, wn, x0, xd0)
	plot!(p, t_vec, x_orig,
	    label     = "ζ = $(zeta0)  (fitted)",
	    color     = :black,
	    lw        = 2.2,
	    linestyle = :dash)
...
manifest_format = "2.0"
project_hash = "5e3e98d8be3ca9472eec4d4199b3dc81e52fc09e"
...
git-tree-sha1 = "56d13cdcf74ec181c9cb86b89380a44818da780b"
...
git-tree-sha1 = "e1001d501251ac9a4929865583910ecfa9c52ce0"
