...
# ╔═╡ aa541eea-403b-4b2c-a199-a89d33e1bccf
using Plots, DifferentialEquations,LaTeXStrings,PlutoUI, FFTW,DSP,ForwardDiff

# ╔═╡ ca4b4f26-8f5d-43b3-b681-b6bc64996ad2
include("./resources/week2/tangential_force.jl");

# ╔═╡ 37f7f6d2-3ce2-4cf7-b6fa-218e95ef16f6
include("./resources/week2/fourier_torque.jl");

# ╔═╡ a6bbd2e0-5989-11f1-aeb3-0925625a1384
md"""
# WEEK 2
"""

# ╔═╡ 35966b7b-3093-4773-95eb-5b01430b16bb
md"""
the typical single pistol engine is radial engine, as shown below
![radial engine]()

since horizontal mode's natural frequency is high enough and less vibration then vertical direction, we only consider vertical vibration in this case. if the motion of engine is stationary enough, neglecting moment force couple and therefore consider only effect by force. For sake of simplicity, if we take equivalent term and assume the total effect by radial engine is same produced by single cylinder.(at least tangentional force, what we are considering most for now)then we can express total base excitation by engine, as given in *Analysis of torsional vibration in internal combustion
engines: modelling and experimental validation)*(DOI: 10.1243/14644193JMBD126)
, we can assume the force is like below.
"""

# ╔═╡ 190721f2-a1a3-4f36-8c0d-3c5875275364
@bind rpm NumberField(0:2e5, default=2e3)

# ╔═╡ 5c236fe5-2a9e-4432-92d4-281b8a7e9cd2
plot_rpm_fig(rpm)

# ╔═╡ 09cc22e0-cd14-48ff-b492-655b559eee4c
r = fourier_decompose(rpm, 24); #return rpm based behavior on upper engine design

# ╔═╡ ebc6bbc7-f1eb-4c08-834f-4b1022d76a61
md"""
Based on the graph above, the horizontal force component can be neglected. The vertical force component is then expressed as a sum of sinusoids via Fourier decomposition (order = 24)."""

# ╔═╡ 8fb0efcb-eff8-4614-9bbf-73bdb8c9e815
r.plot


# ╔═╡ a4c7e5fe-bea9-414a-9afe-490e06624578
md"""
note n/2 since 4 stroke engine require 2 revolution to finish full cycle """

# ╔═╡ 2dd3b37e-511a-4365-8bdb-b3eb5281930b
md"""
Using the Fourier-decomposed excitation ``Y(t)`` derived above and the equation of motion from Week 1,

``m\ddot{x} + c\dot{x} + kx = \sum_{i=1}^{\infty} H(i\omega)\,e^{i\omega t} = Y(t)``

we assume the wing retains the same modal properties as identified in Week 1: ``\zeta = 0.01896``, ``\omega_n = 19.9328`` rad/s.

**For simplicity, the wing mass is fixed at** ``m = 100`` **kg.**
"""

# ╔═╡ 54daf206-657f-4fdb-8a70-48f99fd973da
@bind t_last NumberField(0:0.1:30, default=10)

# ╔═╡ 43e20536-6326-4cac-8fea-5874280d2dbb
begin
	# ── Parameters ───────────────────────────────────────────────────────────────
	wn = 19.3828
	m  = 100
	# mass            [kg]
	c  = 0.01896*(2*m*19.9328)   # damping coeff   [N·s/m]
	k  = wn^2*(m)   # stiffness       [N/m]
	y(t) = 0.5*evaluate_fourier(t,r.omegas,r.coefficients) #[N]
	#! divide by half assume aircraft is fixed wing aircraft.
	
	# ── Equation of motion: m*x'' + c*x' + k*x = y(t) ───────────────────────────
	# State: u = [x, x'] #state space representation
	function eom!(du, u, p, t)
	    m, c, k = p
	    du[1] = u[2]
	    du[2] = (y(t) - c*u[2] - k*u[1]) / m
	end
	
	u0    = [0.0, 0]          # initial conditions: x(0)=0, x'(0)=0
	tspan = (0.0,t_last) 
	p     = (m, c, k)
...
# ── Plot ──────────────────────────────────────────────────────────────────────
t_vec = range(tspan[1], tspan[2], length=2000)
x_vec = [sol(t)[1] for t in t_vec]
y_vec = [y(t)      for t in t_vec]

plt = plot(t_vec, y_vec, label="y(t) [N]", lw=1.5, ls=:dash, color=:orange,
           xlabel="Time [s]", ylabel="Force [N]", legend=:topright,
           title="Damped Forced Vibration  (m=$(m), c=$(round(c)), k=$(round(k)))")
plt2 = twinx(plt)
plot!(plt2, t_vec, x_vec, label="x(t) [m]", lw=2, color=:steelblue,
      ylabel="Displacement [m]", legend=:topleft)

#savefig(ptot,"week2_total.png")
end

# ╔═╡ 6fd24617-17c7-4ecc-b1b7-a851a4b0ba49
begin
	plot(t_vec,y_vec,xlims=(0,1),title=(L"Y(t)"))
	#savefig(p_force,"week2_only_force.png")
	
end

# ╔═╡ 9bada243-2fc2-4712-a5da-03c32c8925db
begin
	plot(t_vec,x_vec,title="displacement")
	#savefig(p_disp,"week2_only_displacement.png")
	
end

# ╔═╡ 157801b1-244f-49f0-aee7-514373a308b1
md"""
## Wing Mass vs. Peak Amplitude — Pareto Frontier
"""

# ╔═╡ dc069521-aba4-4d8b-88da-65c0e0cdc89b
md"""
Assuming the natural frequency is held constant, we investigate how the peak steady-state amplitude varies with wing mass.
"""

# ╔═╡ 17a17e4c-7059-4da9-9cf4-996b343c005e
begin
	m_range = collect(100:100:15000)
	m_max = Vector{Float64}(undef, length(m_range))
	Threads.@threads for i in eachindex(m_range)
		m_i = m_range[i]
		prob_new = remake(prob; p=(m_i, 0.01896*2*m_i*wn, m_i*wn^2))
		sol_ = solve(prob_new, Tsit5(), reltol=1e-8, abstol=1e-8)
		x_vec_ = [sol_(t)[1] for t in t_vec]
		m_max[i] = maximum(abs.(x_vec_))
	end
end

# ╔═╡ faa9a60f-42c7-423d-9847-4e5491c34b0d
begin
	# Brute-force 2D grid: vary mass AND stiffness independently (not tied via ωn).
	# For each (m, k) pair simulate peak amplitude, then extract Pareto-optimal front.
	ζ_bf      = 0.01896
	m_bf_grid = collect(100:500:15000)
	k_bf_grid = exp.(range(log(5e2), log(8e6), length=30))
	all_combos = collect(Iterators.product(m_bf_grid, k_bf_grid))

	bf_results = Vector{Tuple{Float64,Float64}}(undef, length(all_combos))
	Threads.@threads for idx in eachindex(all_combos)
		m_i, k_i = all_combos[idx]
		c_i = 2ζ_bf * sqrt(m_i * k_i)
		sol_i = solve(remake(prob; p=(m_i, c_i, k_i)), Tsit5(), reltol=1e-8, abstol=1e-8)
		x_i = [sol_i(t)[1] for t in t_vec]
		bf_results[idx] = (m_i, maximum(abs.(x_i)))
	end

	# A point (m_i, a_i) is Pareto-optimal if no other point (m_j, a_j) has
	# m_j ≤ m_i AND a_j ≤ a_i with at least one strict inequality.
	bf_pareto = filter(bf_results) do (m_i, a_i)
		!any(bf_results) do (m_j, a_j)
			m_j ≤ m_i && a_j ≤ a_i && (m_j < m_i || a_j < a_i)
		end
	end
	sort!(bf_pareto, by = first)
	bf_m = first.(bf_pareto)
	bf_a = last.(bf_pareto);
end

# ╔═╡ e8a2b4c1-3f5d-4e6b-8a9c-0d1e2f3a4b5c
begin
	# ── Constraints (engineering inputs — set by user) ────────────────────────
	x_limit = 1e-2        # max allowable amplitude [m]  ← adjust to your spec
	m_min   = 100.0       # structural minimum mass  [kg] ← adjust to your spec

	# ── Theoretical 1/m reference calibrated to first data point ─────────────
	C_ref = m_max[1] * m_range[1]
	m_ref_line = 10 .^ range(log10(m_range[1]), log10(m_range[end]), length=300)

	plt_pareto = plot(m_range, m_max,
		xlabel="Mass [kg]", ylabel="Max Amplitude [m]",
		xscale=:log10, yscale=:log10,
		title="Wing Mass vs Peak Amplitude — Pareto Frontier",
		label="Simulated frontier", lw=2, color=:steelblue,
		marker=:circle, markersize=2, legend=:topright)

	# 1/m reference
	plot!(plt_pareto, m_ref_line, C_ref ./ m_ref_line,
		ls=:dash, color=:gray, lw=1.5, label="1/m reference (slope = −1)")

	# amplitude constraint (horizontal)
	hline!(plt_pareto, [x_limit],
		ls=:dot, color=:red, lw=2, label="Amplitude limit $(x_limit) m")

	# mass constraint (vertical)
	vline!(plt_pareto, [m_min],
		ls=:dot, color=:forestgreen, lw=2, label="Min. mass $(m_min) kg")

	# baseline design point
	scatter!(plt_pareto, [m_range[1]], [m_max[1]],
		markersize=8, color=:orange, markershape=:star5, label="Baseline (m=$(m_range[1]) kg)")

	# Brute-force Pareto frontier: non-dominated designs from 2D (mass × stiffness) grid
	plot!(plt_pareto, bf_m, bf_a,
		lw=2, color=:crimson, ls=:dashdot,
		marker=:diamond, markersize=4, label="Brute-force Pareto frontier")

	plt_pareto
end

# ╔═╡ 2be28692-a16d-4556-a39e-d4400207fde9
md"""
This indicate we need to adjust stiffness to get better system
"""

# ╔═╡ fa3cdf7c-9af5-4251-b4a3-bf581ad8d09c
md"""
## Recommended Stiffness k for Flight-Envelope Safety — Engineering Justification
"""
...
In this context, the **flight envelope** refers to the **RPM range** over which the engine operates during normal flight.

Fundamental excitation frequency of a 4-stroke radial engine: ``\omega_{exc} = \frac{\text{RPM} \cdot \pi}{60}`` [rad/s]

If the wing natural frequency ``\omega_n = \sqrt{k/m}`` coincides with this excitation frequency, **resonance** occurs and the vibration amplitude grows rapidly.

**Design principle — Frequency Separation (Soft Design):** Choose ``k`` such that ``\omega_n < \eta \cdot \omega_{exc,min}`` holds across the entire flight envelope. (``\eta = 0.8``, 20% safety margin)
"""

# ╔═╡ b2c3d4e5-f6a7-4b8c-9d0e-1f2a3b4c5d6e
begin
	rpm_flight  = collect(600:50:2700)
	x_peak_rpm  = Vector{Float64}(undef, length(rpm_flight))

	for i in eachindex(rpm_flight)
		r_i    = fourier_decompose(rpm_flight[i], 24)
		oms_i  = copy(r_i.omegas)
		coef_i = copy(r_i.coefficients)
		yi = let oms = oms_i, coef = coef_i
			t -> 0.5 * evaluate_fourier(t, oms, coef)
		end
		prob_i = ODEProblem(
			(du, u, p, t) -> begin
				m_, c_, k_ = p
				du[1] = u[2]
				du[2] = (yi(t) - c_*u[2] - k_*u[1]) / m_
			end,
			[0.0, 0.0], (0.0, 20.0), (m, c, k)
		)
		sol_i  = solve(prob_i, Tsit5(), reltol=1e-6, abstol=1e-6)
		t_ss   = range(15.0, 20.0, length=500)
		x_peak_rpm[i] = maximum(abs.([sol_i(t)[1] for t in t_ss]))
	end
end

# ╔═╡ c3d4e5f6-a7b8-4c9d-0e1f-2a3b4c5d6e7f
begin
	x_safe_env = 5e-2   # allowable peak amplitude [m] — adjust to your spec

	plt_env = plot(rpm_flight, x_peak_rpm,
		xlabel="Engine RPM", ylabel="Steady-state Peak Amplitude [m]",
		title="Amplitude Response across Flight Envelope  (k = $(round(Int, k)) N/m)",
		label="Peak displacement |x(t)|", lw=2, color=:steelblue,
		marker=:circle, markersize=3, legend=:topright)

	hline!(plt_env, [x_safe_env],
		ls=:dash, color=:red, lw=2,
		label="Allowable amplitude limit  $(x_safe_env) m")

	resonant_rpm_1st = wn / (π/60)
	vline!(plt_env, [resonant_rpm_1st],
		ls=:dot, color=:orange, lw=2,
		label="1st resonance RPM ≈ $(round(Int, resonant_rpm_1st))")

	plt_env
	savefig(plt_env,"week2_rpm_relative")
end
...
begin
	...
	ω_exc_min_fe = rpm_min_fe * π / 60
	ω_exc_max_fe = rpm_max_fe * π / 60
	wn_limit_fe  = η_sep * ω_exc_min_fe
	k_rec        = m * wn_limit_fe^2

	(;
		ω_exc_min_rad_s  = round(ω_exc_min_fe, digits=2),
		ω_exc_max_rad_s  = round(ω_exc_max_fe, digits=2),
		wn_limit_rad_s   = round(wn_limit_fe,  digits=2),
		k_recommended    = round(Int, k_rec),
		k_current        = round(Int, k),
		wn_current_rad_s = round(wn, digits=4),
		is_safe          = k <= k_rec
	);
end

# ╔═╡ abfacf7b-e534-43d4-90ef-d008228a9e44
md"""
### Engineering Justification

**1. Flight Envelope Definition**
Operating RPM range for a small piston-engine aircraft: 600 – 2700 RPM
→ Fundamental excitation frequency range: $(round(600*π/60, digits=1)) – $(round(2700*π/60, digits=1)) rad/s


**2. Resonance Avoidance Condition (Frequency Separation)**
``\omega_n < 0.8 \times \omega_{exc,min} = 0.8 \times`` $(round(ω_exc_min_fe, digits=1)) ``\approx`` $(round(wn_limit_fe, digits=1)) rad/s
→ Recommended stiffness upper bound: ``k \leq m \cdot \omega_{n,lim}^2`` = $(round(Int, k_rec)) N/m


**3. Current Design Verification**
$\omega_n$ = $(round(wn, digits=2))rad/s $(k <= k_rec ? "<" : ">") $(round(wn_limit_fe, digits=1))rad/s &&
k = $(round(Int, k)) N/m $(k <= k_rec ? "<" : ">") $(round(Int, k_rec)) N/m

→ $(k <= k_rec ? "**✓ Safe: no resonance occurs across the entire flight envelope**" : "**✗ Unsafe: resonance possible — stiffness must be reduced**")

**4. Conclusion**
Keeping wing stiffness at k ≤ ($(round(Int, k_rec))N/m) 

ensures no resonance occurs anywhere in the flight envelope (600 – 2700 RPM). The current design k $\approx$ $(round(Int, k))  N/m satisfies this criterion.
...
manifest_format = "2.0"
project_hash = "3c88400b05924558f3d63350b74b3ed8ad9703b0"
...
git-tree-sha1 = "795e88027104b6dca1716f5fbda78663f38f51b3"
